...
def resolve_book_paths(book_id: str) -> dict:
    ...
    if settings.IS_PRODUCTION:
        ...
        # In production, database files also come from S3 (no /library/ prefix)
        base_url = settings.DATA_BASE_URL
        if not base_url.endswith('/'):
            base_url += '/'
        paths["db"] = f"{base_url}{book_id}/build/db/booker.db"
        paths["index"] = f"{base_url}{book_id}/build/indexes/booker.faiss"
        paths["meta"] = f"{base_url}{book_id}/build/indexes/booker.pkl"
    else:
